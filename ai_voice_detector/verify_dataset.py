"""
Sanity-checks data/real/ and data/fake/: counts files, tries to load each
with librosa, reports/removes anything unreadable, and prints duration /
sample-rate / class-balance stats.

Usage:
    python verify_dataset.py            # removes corrupted files
    python verify_dataset.py --no-remove  # only flags them, leaves files in place
"""
import argparse
import os
...
def main():
    ...
    print("=" * 60)
    print(f"data/real/: {len(real_stats)} valid files ({len(real_bad)} corrupted)")
    print(f"data/fake/: {len(fake_stats)} valid files ({len(fake_bad)} corrupted)")
